...
public class MegaUltimateLumberjackItem extends DiggerItem implements IModeSwitchable{
    public MegaUltimateLumberjackItem(Tier pTier, Properties pProperties) {
        super(pTier, BlockTags.MINEABLE_WITH_AXE, pProperties);
    }

    @Override
    public int getRange() {
        return 4; /// 9x9x9 area (range=4)
    }

    @Override
    public boolean is3DMining() {
        return true; /// false = 9x9 = 2D, true = 9x9x9 = 3D
    }
    ...
    public List<BlockPos> getBlocksToBeDestroyed(int range, BlockPos initialBlockPos, ServerPlayer player) {
            ...
            for (int x = -range; x <= range; x++) {
                for (int y = -range; y <= range; y++) {  // A destruição vai a partir do bloco para baixo
                    for (int z = -range; z <= range; z++) {
                        positions.add(new BlockPos(initialBlockPos.getX() + x, initialBlockPos.getY() + y + 4, initialBlockPos.getZ() + z));
                    }
                }
            }
        }
            ...
            for (int x = -range; x <= range; x++) {
                for (int y = -1; y <= 7; y++) {
                    for (int z = -range; z <= range; z++) {  // A destruição vai para frente (no eixo Z negativo)
                        positions.add(new BlockPos(initialBlockPos.getX() + x, initialBlockPos.getY() + y, initialBlockPos.getZ() + z + 4));
                    }
                }
            }
        }
        ...
        if (traceResult.getDirection() == Direction.EAST) {
            for (int x = -range; x <= range; x++) {  // A destruição vai para frente (no eixo X positivo)
                for (int y = -1; y <= 7; y++) {
                    for (int z = -range; z <= range; z++) {
                        positions.add(new BlockPos(initialBlockPos.getX() + x - 4, initialBlockPos.getY() + y, initialBlockPos.getZ() + z));
                    }
                }
            }
        }

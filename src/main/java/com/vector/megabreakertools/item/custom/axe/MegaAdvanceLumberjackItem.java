...
public class MegaAdvanceLumberjackItem extends DiggerItem implements IModeSwitchable{
    public MegaAdvanceLumberjackItem(Tier pTier, Properties pProperties) {
        super(pTier, BlockTags.MINEABLE_WITH_AXE, pProperties);
    }

    @Override
    public int getRange() {
        return 2; /// 5x5x5 area (range=2)
    }

    @Override
    public boolean is3DMining() {
        return true; /// false = 5x5 = 2D, true = 5x5x5 = 3D
    }
    ...
    public List<BlockPos> getBlocksToBeDestroyed(int range, BlockPos initialBlockPos, ServerPlayer player) {
            ...
            for (int x = -range; x <= range; x++) {
                for (int y = -range; y <= range; y++) {  // A destruição vai a partir do bloco para baixo
                    for (int z = -range; z <= range; z++) {
                        positions.add(new BlockPos(initialBlockPos.getX() + x, initialBlockPos.getY() + y + 2, initialBlockPos.getZ() + z));
                    }
                }
            }
        }
            ...
            for (int x = -range; x <= range; x++) {
                for (int y = -1; y <= 3; y++) {
                    for (int z = -range; z <= range; z++) {  // A destruição vai para frente (no eixo Z negativo)
                        positions.add(new BlockPos(initialBlockPos.getX() + x, initialBlockPos.getY() + y, initialBlockPos.getZ() + z + 2));
                    }
                }
            }
        }
        ...
        if (traceResult.getDirection() == Direction.EAST) {
            for (int x = -range; x <= range; x++) {  // A destruição vai para frente (no eixo X positivo)
                for (int y = -1; y <= 3; y++) {
                    for (int z = -range; z <= range; z++) {
                        positions.add(new BlockPos(initialBlockPos.getX() + x - 2, initialBlockPos.getY() + y, initialBlockPos.getZ() + z));
                    }
                }
            }
        }
